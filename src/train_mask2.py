...
from models.mask2former_hf import Mask2FormerHF
...
# ====== 配置日志系统 ======
def setup_logger(log_dir='logs', log_name='train_mask2seg'):
    """配置日志记录器，同时输出到文件和控制台"""
    # 创建日志目录
...
def train(use_rgb=True, use_hsv=False, use_lab=False, color_space_name="RGB"):
    os.makedirs('weights_mask2', exist_ok=True)
    ...
    logger.info(f"using model: Mask2Segmentation  backbone")
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_mIoU > best_miou:
            best_miou = avg_mIoU
            model_path = f'weights_mask2/best_{color_space_name}_model.pth'
        ...
        # 每80个epoch保存一次检查点
        if (epoch + 1) % 80 == 0:
            checkpoint_path = f'weights_mask2/checkpoint_{color_space_name}_epoch{epoch+1}.pth'
            torch.save({
                'epoch': epoch + 1,
...
if __name__ == "__main__":
    logger.info("="*60)
    logger.info(" Starting Professional Lovász-Softmax mask2 Training")
    logger.info("="*60)
